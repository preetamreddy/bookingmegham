...
class Tooltip < ActiveRecord::Base
  ...
  validates :title, :content, :markup, presence: true
  validates_uniqueness_of :title, :scope => :locale
  validates_format_of :locale, :with => /^[a-z][a-z]$/,
    :message => 'must be in ISO-639-1 two letter format (e.g. en, de, zh)',
    :allow_blank => true
  validates_inclusion_of  :markup, :in => %w{ html markdown textile },
  ...
  def self.from_title_and_locale(title, user_locale = I18n.locale.to_s)
    tooltip = nil
    tooltips = Tooltip.find_all_by_title(title)
